...
// ********************
int FlatSmoothMode =
    0; // ==0 for rendering in smooth mode, ==1 for rendering in flat mode
...
unsigned int myVAO[NumObjects]; // a Vertex Array Object - holds info about an
                                // array of vertex data;

// We create two shader programs: each consists of a vertex shader and a
// fragment shader
...
int projMatLocation_smooth; // Location of the projectionMatrix in the "smooth"
                            // shader program.
int projMatLocation_flat;   // Location of the projectionMatrix in the "flat"
                            // shader program.
const char *modelviewMatName =
    "modelviewMatrix";           // Name of the uniform variable modelviewMatrix
int modelviewMatLocation_smooth; // Location of the modelviewMatrix in the
                                 // "smooth" shader program.
int modelviewMatLocation_flat; // Location of the modelviewMatrix in the "flat"
                               // shader program.
...
void mySetupGeometries() {
  ...
  // Allocate the needed Vertex Array Objects (VAO's) and Vertex Buffer Objects
  // (VBO')
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(triangleFanVerts), triangleFanVerts,
               GL_STATIC_DRAW);
  ...
  //          These positions were just loaded into the VBO as part of the above
  //          array, one per vertex.
  //  vertColor_loc corresponds to the "location = 1" in the vertex shader.
  //			These colors were also just loaded into the VBO, one
  //color per vertex.
  // The next four commands tell the VAO where the vertex positions and color
  // values
  //          are stored in the VBO.
  // The value "6 * sizeof(float)" is the "stride" (distance between buffer
  // locations where values are stored. The value "3 * sizeof(float)" is the
  // offset in the VBO buffer where the first color start.
  glVertexAttribPointer(vertPos_loc, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float),
                        (void *)0);       // Vertex data is in the VBO
  glEnableVertexAttribArray(vertPos_loc); // Enable use of the vertex data
  glVertexAttribPointer(vertColor_loc, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float),
                        (void *)(3 * sizeof(float)));
  glEnableVertexAttribArray(vertColor_loc);

  // The model view matrix for triangle fan resizes and repositions it
  theModelViewMatrixTriFan.Set_glTranslate(
      1.2, 1.2, 0.0); // Initialize to translation by (1.2, 1.2, 0.0)
  theModelViewMatrixTriFan.Mult_glScale(
      0.5); // Shrink using scale factor 1/2 (multiplies on the right)
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(triangleStripVerts), triangleStripVerts,
               GL_STATIC_DRAW);
  glVertexAttribPointer(vertPos_loc, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float),
                        (void *)0);
  ...
  theModelViewMatrixTriStrip.Set_glTranslate(
      -1.5, 0.5, 0.0); // Initialize to translation by (-1.5, 0.5, 0.0)
  theModelViewMatrixTriStrip.Mult_glScale(0.4); // Shrink by scaling factor 0.4
  theModelViewMatrixTriStrip.Mult_glRotate(
      -0.785, 0.0, 0.0, 1.0); // Rotate 45 degrees (0.785 radians) clockwise
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(trianglesVerts), trianglesVerts,
               GL_STATIC_DRAW);
  glVertexAttribPointer(vertPos_loc, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float),
                        (void *)0);
  glEnableVertexAttribArray(vertPos_loc);
...
void myRenderScene() {
  ...
  // Draw three overlapping triangles - Rotate them slightly in each
  // re-rendering pass.
  ...
  mat.Mult_glRotate(
      ...
      -1.0); // Rotate around negative z-axis (clockwise for viewer)
...
void my_setup_SceneData() {
  ...
  // programs.
  projMatLocation_smooth =
      glGetUniformLocation(shaderProgramSmooth, projMatName);
  projMatLocation_flat = glGetUniformLocation(shaderProgramFlat, projMatName);
  modelviewMatLocation_smooth =
      glGetUniformLocation(shaderProgramSmooth, modelviewMatName);
  modelviewMatLocation_flat =
      glGetUniformLocation(shaderProgramFlat, modelviewMatName);
...
void key_callback(GLFWwindow *window, int key, int scancode, int action,
  ...
  if (key == GLFW_KEY_ESCAPE || key == GLFW_KEY_X) {
    ...
    FlatSmoothMode =
        1 -
        FlatSmoothMode; // Toggle between 0 and 1 for flat and smooth shading
  }
}
...
void window_size_callback(GLFWwindow *window, int width, int height) {
  ...
  // window. The complication is that the aspect ratio of the window may not
  // match the
  ...
  theProjectionMatrix.Set_glOrtho(windowXmin, windowXmax, windowYmin,
                                  windowYmax, Zmin, Zmax);
...
int main() {
  ...
  GLFWwindow *window =
      glfwCreateWindow(initWidth, initHeight, "SimpleAnimModern", NULL, NULL);
  if (window == NULL) {
  ...
  glEnable(
      GL_MULTISAMPLE); // Usually is enabled by default, but call just in case.
  ...
  printf("------------------------------\n");
  printf(
      "Press space bar to toggle between flat shading and smooth shading.\n");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glfwWaitEventsTimeout(
        1.0 /
        60.0); // Use this to animate at 60 frames/sec (timing is NOT reliable)
               // glfwWaitEvents();					// Use this if
    // no animation. glfwPollEvents();					// Use
    // this version when animating as fast as possible
  }
