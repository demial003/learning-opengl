...
public:
  ...
  //   Alternately, GetNumElementsMax() and GetNumElementsRender() can be
  //   defined. GetNumElementsMax() returns an upper bound on the number of
  //   elements in the EBO (for allocation) GetNumElementsRender() returns the
  //   actual number of elements in the EBO (for rendering)
  //   GetNumVerticesTexCoords() returns the number of vertices when there are
  //   texture coordinates GetNumVerticesNoTexCoords() returns the number of
  //   vertices when no texture coordinates
  ...
  //       - The VBO and EBO buffersare filled with the vertex info and elements
  //       for GL_TRIANGLES drawing
  ...
  //   vertTexCoordsOffset and stride control where the texture coordinates are
  //   placed. Offset and stride values are **integers** (not bytes), measuring
  //   offsets in terms of floats. Use "-1" for the offset for any value which
  //   should be omitted. For the (unit) sphere, the normals are always exactly
  //   equal to the positions.
  // Output:
  //   Data VBO and EBO data is calculated and loaded into the two buffers
  //   VBOdataBuffer and EBOdataBuffer.
  // Typical usages are:
  //   CalcVboAndEbo( vboPtr, eboPtr, 0, -1, -1, 3); // positions only, tightly
  //   packed CalcVboAndEbo( vboPtr, eboPtr, 0, -1, 3, 5); // positions, then
  //   (s,t) texture coords, tightly packed CalcVboAndEbo( vboPtr, eboPtr, 0, 3,
  //   6, 8); // positions, normals, then (s,t) texture coords, tightly packed
  virtual void CalcVboAndEbo(float *VBOdataBuffer, unsigned int *EBOdataBuffer,
                             int vertPosOffset, int vertNormalOffset,
                             int vertTexCoordsOffset, unsigned int stride) = 0;
...
protected:
  ...
  // First parameter is the location for the vertex position vector in the
  // shader program. Second parameter is the location for the vertex normal
  // vector in the shader program. Third parameter is the location for the
  // vertex 2D texture coordinates in the shader program. The second and third
  // parameters are optional.
  virtual void InitializeAttribLocations(unsigned int pos_loc,
                                         unsigned int normal_loc = UINT_MAX,
                                         unsigned int texcoords_loc = UINT_MAX);
  void ReInitializeAttribLocations();
  void CalcVBOandEBO_Base();

  void PreRender();
  void Render();
  void RenderElements(unsigned int drawMode, int numRenderElements,
                      const unsigned int *elementsData);
...
private:
  ...
  unsigned int
      posLoc; // location of vertex position x,y,z data in the shader program
  unsigned int
      normalLoc; // location of vertex normal data in the shader program
  unsigned int texcoordsLoc; // location of s,t texture coordinates in the
                             // shader program.
