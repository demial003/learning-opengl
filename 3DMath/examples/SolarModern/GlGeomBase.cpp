...
void GlGeomBase::InitializeAttribLocations(unsigned int pos_loc,
                                           unsigned int normal_loc,
                                           unsigned int texcoords_loc) {
  posLoc = pos_loc;
  ...
  int numVertices =
      UseTexCoords() ? GetNumVerticesTexCoords() : GetNumVerticesNoTexCoords();
  glBufferData(GL_ARRAY_BUFFER, StrideVal() * numVertices * sizeof(float), 0,
               GL_STATIC_DRAW);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, theEBO);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               GetNumElementsMax() * sizeof(unsigned int), 0, GL_STATIC_DRAW);
  glVertexAttribPointer(posLoc, 3, GL_FLOAT, GL_FALSE,
                        StrideVal() * sizeof(float), (void *)0);
  glEnableVertexAttribArray(posLoc);
  if (UseNormals()) {
    glVertexAttribPointer(normalLoc, 3, GL_FLOAT, GL_FALSE,
                          StrideVal() * sizeof(float),
                          (void *)(NormalOffset() * sizeof(float)));
    glEnableVertexAttribArray(normalLoc);
  }
  if (UseTexCoords()) {
    glVertexAttribPointer(texcoordsLoc, 2, GL_FLOAT, GL_FALSE,
                          StrideVal() * sizeof(float),
...
void GlGeomBase::PreRender() {
  if (theVAO == 0) {
    assert(false &&
           "InitializeAttribLocations must be called before rendering!");
  }
}
...
// **********************************************
void GlGeomBase::RenderEBO(unsigned int drawMode, int numRenderElements,
                           int EBOstart) {
  if (theVAO == 0) {
    assert(false &&
           "InitializeAttribLocations must be called before rendering!");
  ...
  glBindVertexArray(theVAO);
  glDrawElements(drawMode, (GLsizei)numRenderElements, GL_UNSIGNED_INT,
                 (void *)(EBOstart * sizeof(unsigned int)));
  glBindVertexArray(
      0); // Good practice to unbind: helps with debugging if nothing else
}
...
// **********************************************
void GlGeomBase::RenderElements(unsigned int drawMode, int numRenderElements,
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               numRenderElements * sizeof(unsigned int), elementsData,
               GL_STATIC_DRAW);
  ...
  glBindBuffer(
      ...
      theEBO); // Restore the main EBO (The VAO maintains its knowledge of this)
