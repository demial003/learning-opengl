...
//   It copies the color to "theColor" so that the fragment shader can access
//   it.
const char *vertexShader_PosColorOnly =
    "#version 330 core\n"
    "layout (location = 0) in vec3 vertPos;	   // Position in attribute "
    "location 0\n"
    "layout (location = 1) in vec3 vertColor;  // Color in attribute location "
    "1\n"
    "out vec3 theColor;					       // Output a "
...
const char *fragmentShader_ColorOnly =
    "#version 330 core\n"
    "in vec3 theColor;		// Color value came from the vertex shader "
    "(smoothed) \n"
...
void setup_shaders() {
  ...
  shaderProgram1 = setup_shader_vertfrag(vertexShader_PosColorOnly,
                                         fragmentShader_ColorOnly);
}
 ...
 */
unsigned int setup_shader_vertfrag(const char *vertexShaderSource,
  ...
  // Deallocate shaders since we do not need to use these for other shader
  // programs.
  glDeleteShader(vertexShader);
  glDeleteShader(fragmentShader);

  return shaderProgram; // Return the compiled shaders as a single shader
                        // program.
}
